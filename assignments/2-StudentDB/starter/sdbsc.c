...
int get_student(int fd, int id, student_t *s) {
    ...
    while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
        if (student.id == id) {
            memcpy(s, &student, sizeof(student_t));
...
int del_student(int fd, int id) {
    ...
    ssize_t bytes_read = read(fd, &current_student, STUDENT_RECORD_SIZE);
    if (bytes_read < 0) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    if (memcmp(&current_student, &(student_t){0}, sizeof(student_t)) == 0) {
...
int count_db_records(int fd) {
    ...
    while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
        if (memcmp(&student, &(student_t){0}, sizeof(student_t)) != 0 && student.id > 0) {
            student_count++;
        }
    }
    ...
    printf(M_DB_RECORD_CNT, student_count);
