...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    cmd_buff = (char *)malloc(CMD_BUFF_SIZE);
    rsp_buff = (char *)malloc(RSP_BUFF_SIZE);
    ...
    while (1)
    {
        ...
        if (!fgets(cmd_buff, CMD_BUFF_SIZE, stdin))
        {
            perror("fgets failed");
            break;
        }
        ...
        do
        {
            io_size = recv(cli_socket, rsp_buff, RSP_BUFF_SIZE - 1, 0);
            if (io_size < 0)
            {
                perror("recv failed");
                return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
