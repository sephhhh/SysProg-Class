...
int exec_client_requests(int cli_socket) {
    ...
    while ((bytes_read = read(cli_socket, buffer, sizeof(buffer) - 1)) > 0) {
        buffer[bytes_read] = '\0'; 

        if (strncmp(buffer, "cd ", 3) == 0) {
            printf("Entered 'cd' block. Attempting to change directory...\n");
            char *dir = buffer + 3; 
            
            if (chdir(dir) != 0) {
                send(cli_socket, "cd failed\n", strlen("cd failed\n"), 0);
            } else {
                send(cli_socket, "cd successful\n", strlen("cd successful\n"), 0);
            }
        } else {
            rc = build_cmd_list(buffer, &clist);  
            if (rc < 0) {
...
int send_message_string(int cli_socket, char *buff) {
    int bytes_sent;
    int message_length = strlen(buff);
    
    bytes_sent = send(cli_socket, buff, message_length, 0);
    if (bytes_sent < 0) {
        perror("send failed");
        return ERR_RDSH_COMMUNICATION;
    }

    char eof_char = '\0';  // or you could use a specific EOF byte
    bytes_sent = send(cli_socket, &eof_char, sizeof(eof_char), 0);
    if (bytes_sent < 0) {
